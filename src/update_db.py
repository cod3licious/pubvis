...
from .nlputils_features import FeatureTransform, features2mat, norm_whitespace
...
class Update_similarities(Command):
    ...
    def run(self):
        ...
        print "[INFO] Update_similarities: performing LSA - explained variance:",
    ...
    def run(self):
        ...
        for keyword in ['brain cancer', 'breast cancer', 'colorectal cancer', 'kidney cancer', 'leukemia',
            ...
            pmidquery = baseurl + "esearch.fcgi?" + \
                urllib.urlencode({"db": "pubmed", "term": keyword, "retmax": 1000, "sort": "relevance"})
            # retrieve xml
            urlreq = urllib2.Request(pmidquery, None, {"User-Agent": "python-pubmed%i" % randint(100, 999)})
            xml = urllib2.urlopen(urlreq).read()
            soup = bs(xml, "xml")
            ...
            for i, article_id in enumerate(idlist):
                ...
                if not item:
                    try:
                        # get pubmed summary
                        pmquery = baseurl + "efetch.fcgi?" + urllib.urlencode({"db": "pubmed", "id": article_id, "retmode": "xml"})
                        # retrieve xml
                        urlreq = urllib2.Request(pmquery, None, {"User-Agent": "python-pmc%i" % randint(100, 999)})
                        xml = urllib2.urlopen(urlreq).read()
                        soup = bs(xml, "xml")
                        ...
                        article_data["authors"] = u", ".join([u"%s %s" % (a.find("ForeName").get_text(), a.find(
                        ...
                        if len(article_data.keys()) < 5:
                            print("[ERROR] Fetch_pubmed: Something went wrong downloading article id '%s': %s; obtained:" % (article_id, e)),
                            print article_data.keys()
                # don't download if it already exist, then maybe just update the keyword
                elif item.keywords and not keyword in item.keywords.split(','):
...
class Fetch_arxiv(Command):
    ...
    def run(self):
        ...
        for i in range(start_index, max_index, results_per_iteration):
            ...
            urlreq = urllib2.Request(arxiv_url, None, {"User-Agent": "python-arxiv%i" % randint(100, 999)})
            ...
            for e in parse.entries:
                ...
                if not item or e['id'] > item.item_url:
                    try:
                        ...
                        article_data['authors'] = u", ".join([a['name'] for a in e['authors']])
